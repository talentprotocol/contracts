import { ethers, upgrades, waffle } from "hardhat";

import * as TalentTokenArtifactV2 from "../../artifacts/contracts/test/TalentTokenV2.sol/TalentTokenV2.json";
import * as TalentTokenMigration from "../../artifacts/contracts/season3/TalentTokenMigration.sol/TalentTokenMigration.json";
import { UpgradeableBeacon__factory } from "../../typechain-types";

const { exit } = process;
const { deployContract } = waffle;

async function main() {
  const [owner] = await ethers.getSigners();

  console.log("owner: ", owner.address);

  const factory = await ethers.getContractAt("TalentFactoryV3", "0x8ee4f3044Ef0166A6DB12b0e9Eeb1735f1Fc7cc9");

  // const beaconAddr = await factory.implementationBeacon();

  // console.log("beacon address: ", beaconAddr);

  // const talentTokenV2 = await deployContract(owner, TalentTokenArtifactV2, []);

  // console.log("New talent token contract: ", talentTokenV2.address);
  // console.log("Connecting to beacon");
  // const beacon = UpgradeableBeacon__factory.connect(beaconAddr, owner);

  // console.log("Upgrading");
  // await beacon.upgradeTo(talentTokenV2.address);

  // console.log("Upgrade done");

  const token = await ethers.getContractAt("TalentTokenV2", "0xbbc37ea38ec834ebf296fee21d059f0528eebdd3");

  console.log("mintingAvailability", await token.mintingAvailability());
  console.log("talent", await token.talent());
  console.log("factory", await token.factory());

  const allTokens: any = [
    "0xbbc37ea38ec834ebf296fee21d059f0528eebdd3",
    "0x543766fc6ab468719ad69d8dc2134b127f56865a",
    "0x615170a6d049c037dad71c1d6fa190ac7f9d903e",
    "0xdbafa5dd56e582bb15805a7b665e5183e82459ca",
    "0xf942f5f6a484d5a2aec915dc133e439f1aa5f2cb",
    "0x9c377f7d83f3f3d1cebbb158a4691344f1f7e17a",
    "0x6608e34fc60e3022111084dd5dea5cd2700745fc",
    "0x08da06e515c801865d6be1008b014cb92141e394",
    "0x5cabd4c99388234b0bf797428a1579254b9effd5",
    "0x9954a7ff41da8ce2bda065d0d1e7096a89fb22a5",
    "0x12bde07008bae57715a18ed7451cbef08c855266",
    "0xecc48d4483109ecef307a300d8acb14893c4d311",
    "0x1cf39cdcb086112bda0f09aebca53ed0c477c1fa",
    "0x9e4c2d1c9ae74771d435b8067eb879132c3c6a2b",
    "0x55d1d8ded59c048e1355ec447fb30a1b329da920",
  ];

  // for await (const item of allTokens) {
  //   const token = await ethers.getContractAt("TalentTokenMigration", item);

  //   console.log("Migrating token:", item);

  //   console.log("setInternalState");
  //   const tx = await token
  //     .connect(owner)
  //     .setInternalState(factory.address, "0x5a6eF881E3707AAf7201dDb7c198fc94B4b12636");
  //   await tx.wait();

  //   console.log("Done.");
  // }
}

main()
  .then(() => exit(0))
  .catch((error) => {
    console.error(error);
    exit(1);
  });
