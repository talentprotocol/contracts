...
export default class AuthCommand extends Command {
    ...
    validateStudioDeployKey(value) {
        if (!value)
            return false;
        return /^[0-9a-fA-F]{32}$/.test(value);
    }
    async run() {
        const { args: { 'deploy-key': initialDeployKey }, } = await this.parse(AuthCommand);
        const { node } = chooseNodeUrl({});
        const { deployKey } = await prompt.ask([
            {
                type: 'input',
                name: 'deployKey',
                ...
                validate: value => this.validateStudioDeployKey(value) || `Invalid Subgraph Studio deploy key: ${value}`,
            },
        ]);
        try {
