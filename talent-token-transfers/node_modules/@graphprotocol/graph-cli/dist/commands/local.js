...
export default class LocalCommand extends Command {
    static description = 'Runs local tests against a Graph Node environment (using Ganache by default).';
    static args = {
...
const indent = (indentation, str) => str
    ...
    .map(s => s.replace(/^\s+$/g, ''))
    .join('\n');
const configureTestEnvironment = async (tempdir, composeFile, nodeImage) => await withSpinner(`Configure test environment`, `Failed to configure test environment`, `Warnings configuring test environment`, async () => {
...
const startTestEnvironment = async (tempdir) => await withSpinner(`Start test environment`, `Failed to start test environment`, `Warnings starting test environment`, async (_spinner) => {
    // Bring up the test environment
    await compose.upAll({
        cwd: path.join(tempdir, 'compose'),
    });
});
const waitForTestEnvironment = async ({ skipWaitForEthereum, skipWaitForIpfs, skipWaitForPostgres, timeout, }) => await withSpinner(`Wait for test environment`, `Failed to wait for test environment`, `Warnings waiting for test environment`, async (spinner) => {
...
const stopTestEnvironment = async (tempdir) => await withSpinner(`Stop test environment`, `Failed to stop test environment`, `Warnings stopping test environment`, async () => {
...
const startGraphNode = async (standaloneNode, standaloneNodeArgs, nodeOutputChunks) => await withSpinner(`Start Graph node`, `Failed to start Graph node`, `Warnings starting Graph node`, async (spinner) => {
    const defaultArgs = [
        '--ipfs',
        'localhost:15001',
        '--postgres-url',
...
const waitForGraphNode = async (timeout) => await withSpinner(`Wait for Graph node`, `Failed to wait for Graph node`, `Warnings waiting for Graph node`, async () => {
    await waitFor(timeout, async () => new Promise((resolve, reject) => {
        http
...
const stopGraphNode = async (nodeProcess) => await withSpinner(`Stop Graph node`, `Failed to stop Graph node`, `Warnings stopping Graph node`, async () => {
    nodeProcess.kill(9);
});
...
const runTests = async (testCommand) => await withSpinner(`Run tests`, `Failed to run tests`, `Warnings running tests`, async () => new Promise(resolve => {
    const output = [];
