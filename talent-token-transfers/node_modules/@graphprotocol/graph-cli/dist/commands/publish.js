...
export default class PublishCommand extends Command {
    ...
    async publishWithBrowser({ ipfsHash, webapp, subgraphId, protocolNetwork, apiKey, }) {
        const { openBrowser } = await prompt.ask([
            {
                type: 'confirm',
                name: 'openBrowser',
        ...
        print.success(`Finalize the publish of the subgraph from the Graph CLI publish page. Opening up the browser to continue publishing at ${openUrl}`);
        await open(openUrl);
        return;
    }
    async run() {
