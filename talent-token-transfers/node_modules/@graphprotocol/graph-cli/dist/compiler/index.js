...
export default class Compiler {
    ...
    constructor(options) {
        ...
        for (let dir = path.resolve(this.sourceDir); 
    ...
    async loadSubgraph({ quiet } = { quiet: false }) {
        ...
        return await withSpinner(`Load subgraph from ${manifestPath}`, `Failed to load subgraph from ${manifestPath}`, `Warnings loading subgraph from ${manifestPath}`, async () => {
    ...
    _compileDataSourceMapping(protocol, dataSource, mappingPath, compiledFiles, spinner, validate = false) {
        ...
        try {
            ...
            if (alreadyCompiled) {
                ...
                step(spinner, 'Compile data source:', `${dataSourceName} => ${this.displayPath(outFile)} (already compiled)`);
                return outFile;
            }
            const outFile = path.resolve(this.subgraphDir(this.options.outputDir, dataSource), this.options.outputFormat == 'wasm' ? `${dataSourceName}.wasm` : `${dataSourceName}.wast`);
    ...
    _compileTemplateMapping(template, mappingPath, compiledFiles, spinner) {
        try {
            ...
            if (alreadyCompiled) {
                const outFile = compiledFiles.get(inputCacheKey);
                step(spinner, 'Compile data source template:', `${templateName} => ${this.displayPath(outFile)} (already compiled)`);
                return outFile;
            }
            const outFile = path.resolve(this.options.outputDir, 'templates', templateName, this.options.outputFormat == 'wasm' ? `${templateName}.wasm` : `${templateName}.wast`);
            step(spinner, 'Compile data source template:', `${templateName} => ${this.displayPath(outFile)}`);
            const outputFile = path.relative(baseDir, outFile);
            // Create output directory
            fs.mkdirsSync(path.dirname(outFile));
            const libs = this.libsDirs.join(',');
            ...
            subgraph = subgraph.update('dataSources', (dataSources) => dataSources.map(dataSource => {
                ...
                if (this.protocol.hasABIs()) {
                    updatedDataSource = updatedDataSource
                        ...
                        return path.relative(this.options.outputDir, this._writeSubgraphFile(abiFile, JSON.stringify(abiData.data.toJS(), null, 2), this.sourceDir, this.subgraphDir(this.options.outputDir, dataSource), spinner));
                    })));
                }
                if (protocol.name == 'substreams' || protocol.name == 'substreams/triggers') {
                    updatedDataSource = updatedDataSource
                        ...
                        return path.relative(this.options.outputDir, this._writeSubgraphFile(packageFile, packageContent, this.sourceDir, this.subgraphDir(this.options.outputDir, dataSource), spinner));
                    }));
                    if (updatedDataSource.getIn(['mapping', 'file'])) {
                        updatedDataSource = updatedDataSource.updateIn(['mapping', 'file'], (mappingFile) => path.relative(this.options.outputDir, path.resolve(this.sourceDir, mappingFile)));
                    }
                    return updatedDataSource;
                }
                // The mapping file is already being written to the output
                // directory by the AssemblyScript compiler
                return updatedDataSource.updateIn(['mapping', 'file'], (mappingFile) => path.relative(this.options.outputDir, path.resolve(this.sourceDir, mappingFile)));
            }));
                ...
                : templates.map((template) => {
                    ...
                    if (this.protocol.hasABIs()) {
                        updatedTemplate = updatedTemplate
                            ...
                            abiFile = path.resolve(this.sourceDir, abiFile);
                            const abiData = this.ABI.load(abi.get('name'), abiFile);
                            return path.relative(this.options.outputDir, this._writeSubgraphFile(abiFile, JSON.stringify(abiData.data.toJS(), null, 2), this.sourceDir, this.subgraphDir(this.options.outputDir, template), spinner));
                        })));
                    }
                    // The mapping file is already being written to the output
                    // directory by the AssemblyScript compiler
                    return updatedTemplate.updateIn(['mapping', 'file'], (mappingFile) => path.relative(this.options.outputDir, path.resolve(this.sourceDir, mappingFile)));
                }));
    ...
    async uploadSubgraphToIPFS(subgraph) {
        return withSpinner(`Upload subgraph to IPFS`, `Failed to upload subgraph to IPFS`, `Warnings while uploading subgraph to IPFS`, async (spinner) => {
