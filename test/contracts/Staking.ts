...
describe("Staking", () => {
  ...
  describe("upgradabled", () => {
    ...
    it("allows upgrading the factory itself", async () => {
      const TalentFactoryV2Factory = (await ethers.getContractFactory(
        "TalentFactoryV2"
      )) as TalentFactoryV2__factory;
      const factory2 = (await upgrades.upgradeProxy(
        factory,
        TalentFactoryV2Factory
      )) as TalentFactoryV2;
    ...
    it("allows upgrading the staking contract", async () => {
      ...
      const staking2 = (await upgrades.upgradeProxy(
        staking,
        StakingV2Factory
      )) as StakingV2;
  ...
  describe("functions", () => {
    ...
    describe("stakeStable", () => {
      it("accepts stable coin stakes", async () => {
        await stable
          .connect(investor1)
          .approve(staking.address, parseUnits("25"));

        const investorStableBalanceBefore = await stable.balanceOf(
          investor1.address
        );
        const action = staking
          .connect(investor1)
          .stakeStable(talentToken1.address, parseUnits("25"));

        await expect(action).not.to.be.reverted;

        // USDT is deducted
        expect(await stable.balanceOf(investor1.address)).to.eq(
          investorStableBalanceBefore.sub(parseUnits("25"))
        );

        const talentBalance = await talentToken1.balanceOf(investor1.address);
        const expectedBalance = await staking.convertUsdToTalent(
          parseUnits("25")
        );
      ...
      it("accepts stable coin stakes with decimals", async () => {
        await stable
          .connect(investor1)
          .approve(staking.address, parseUnits("0.0099"));
        ...
        const action = staking
          .connect(investor1)
          .stakeStable(talentToken1.address, parseUnits("0.0099"));

        await expect(action).not.to.be.reverted;

        // USDT is deducted
        expect(await stable.balanceOf(investor1.address)).to.eq(
          investorStableBalanceBefore.sub(parseUnits("0.0099"))
        );
      ...
      it("creates a stake", async () => {
        ...
        const stake = await staking.stakes(
          investor1.address,
          talentToken1.address
        );

        expect(stake.tokenAmount).to.equal(
          await staking.convertUsdToToken(parseUnits("25"))
        );
      });
      ...
      it("emits the expected Stake event", async () => {
        await stable
          .connect(investor1)
          .approve(staking.address, parseUnits("1"));
      ...
      it("updates totalTokensStaked", async () => {
        ...
        await staking
          .connect(investor2)
          .stakeStable(talentToken2.address, parseUnits("1"));
      ...
      it("staking twice in the same talent goes through a checkpoint", async () => {
        await stable
          .connect(investor1)
          .approve(staking.address, parseUnits("50"));

        await staking
          .connect(investor1)
          .stakeStable(talentToken1.address, parseUnits("25"));
        const stakeBefore = await staking.stakes(
          investor1.address,
          talentToken1.address
        );
        ...
        await staking
          .connect(investor1)
          .stakeStable(talentToken1.address, parseUnits("25"));
        const stakeAfter = await staking.stakes(
          investor1.address,
          talentToken1.address
        );

        expect(stakeAfter.lastCheckpointAt).to.be.gt(
          stakeBefore.lastCheckpointAt
        );

        expect(stakeBefore.talentAmount).to.eq(parseUnits("250"));
        expect(stakeAfter.talentAmount).to.be.closeTo(
          parseUnits("500"),
          margin
        );
      });
      ...
      it("staking twice in different talents does not go through a checkpoint", async () => {
        ...
        await staking
          .connect(investor1)
          .stakeStable(talentToken1.address, parseUnits("25"));
        const stake1Before = await staking.stakes(
          investor1.address,
          talentToken1.address
        );
        ...
        await staking
          .connect(investor1)
          .stakeStable(talentToken2.address, parseUnits("25"));
        const stake1After = await staking.stakes(
          investor1.address,
          talentToken1.address
        );

        const stake2 = await staking.stakes(
          investor1.address,
          talentToken1.address
        );
        ...
        expect(stake1After.tokenAmount).to.equal(stake2.tokenAmount);
        expect(stake1After.lastCheckpointAt).to.eq(
          stake1Before.lastCheckpointAt
        );
      });
      ...
      it("fails if stake exceeds mintingAvailability", async () => {
        await stable
          .connect(owner)
          .approve(staking.address, await stable.balanceOf(owner.address));

        const action = staking
          .connect(owner)
          .stakeStable(
            talentToken1.address,
            await stable.balanceOf(owner.address)
          );

        await expect(action).to.be.revertedWith(
          "_amount exceeds minting availability"
        );
      });
    });
    ...
    describe("swapStableForToken", () => {
      it("swaps existing stable coin for TAL", async () => {
        ...
        await staking
          .connect(investor1)
          .stakeStable(talentToken1.address, stableAmount);
        ...
        expect(await tal.balanceOf(owner.address)).to.equal(
          initialOwnerTalBalance.sub(tokenAmount)
        );

        expect(await stable.balanceOf(staking.address)).to.equal(0);
        expect(await stable.balanceOf(owner.address)).to.equal(
          initialOwnerStableBalance.add(stableAmount)
        );
      });
      ...
      it("does not accept withdrawing more stable coin than available", async () => {
        ...
        const action = staking
          .connect(owner)
          .swapStableForToken(parseUnits("50"));
    ...
    describe("claimRewards", () => {
      it("emits a RewardClaim event and updates the stake", async () => {
        ...
        const tx = await staking
          .connect(investor1)
          .claimRewards(talentToken1.address);
        ...
        expect(
          event?.args?.stakerReward.add(event?.args?.talentReward)
        ).to.be.closeTo(rewards, margin);
        ...
        const talentRedeemable = await staking.talentRedeemableRewards(
          talentToken1.address
        );
    ...
    describe("withdrawRewards", () => {
      it("sends rewards to the owner", async () => {
        ...
        const tx = await staking
          .connect(investor1)
          .withdrawRewards(talentToken1.address);
        ...
        expect(balanceAfter).to.eq(
          balanceBefore.add(event?.args?.stakerReward)
        );
    ...
    describe("withdrawTalentRewards", () => {
      it("allows talent to withdraw his redeemable share", async () => {
        ...
        await staking
          .connect(talent1)
          .withdrawTalentRewards(talentToken1.address);
    ...
    describe("stakingAvailability", () => {
      it("shows how much TAL can be staked in a token", async () => {
        ...
        const amount = parseUnits("50");
        const amountBefore = await staking.stakeAvailability(
          talentToken1.address
        );
        await transferAndCall(
          tal,
        ...
        const amountAfter = await staking.stakeAvailability(
          talentToken1.address
        );

        expect(amountAfter).to.equal(amountBefore.sub(amount));

        expect(amountAfter).to.eq(
          await staking.convertTalentToToken(
            await talentToken1.mintingAvailability()
          )
        );
      });
      ...
      describe("onTransferReceived", () => {
        describe("TAL stakes", () => {
          ...
          it("allows creating stakes in different talents", async () => {
            ...
            await transferAndCall(
              ...
              staking.address,
              parseUnits("100"),
              talentToken2.address
            );

            const stake1 = await staking.stakes(
              investor1.address,
              talentToken1.address
            );
            const stake2 = await staking.stakes(
          ...
          it("updates totalTokensStaked", async () => {
            ...
            expect(await staking.totalTokensStaked()).to.equal(
              parseUnits("50")
            );
          ...
          it("rejects TAL stakes while not yet", async () => {
            ...
            await expect(action).to.be.revertedWith(
              "Unrecognized ERC1363 token received"
            );
          });
          ...
          it("accepts TAL stakes in the second phase", async () => {
            ...
            const investorTalBalanceBefore = await tal.balanceOf(
              investor1.address
            );
            const action = transferAndCall(
              tal,
            ...
            // TAL is deducted
            expect(await tal.balanceOf(investor1.address)).to.eq(
              investorTalBalanceBefore.sub(parseUnits("50"))
            );

            const talentBalance = await talentToken1.balanceOf(
              investor1.address
            );
            const expectedBalance = await staking.convertTokenToTalent(
              parseUnits("50")
            );
        ...
        describe("Talent Token refunds", () => {
          ...
          it("accepts Talent Tokens in the second phase, to refund a TAL investment", async () => {
            ...
            expect(await talentToken1.balanceOf(investor1.address)).to.equal(
              parseUnits("1")
            );
          ...
          it("deducts from totalTokensStaked", async () => {
            ...
            expect(totalAfter).to.be.closeTo(
              totalBefore.sub(parseUnits("2.5")),
              margin
            );
          });
          ...
          it("performs a checkpoint and keeps a stake with the remainder", async () => {
            ...
            expect(stakeAfter.tokenAmount).to.be.closeTo(
              parseUnits("5"),
              margin
            );
          });
    ...
    describe("stableCoinBalance", () => {
      it("returns the amount of stable coin held", async () => {
        await stable
          .connect(investor1)
          .transfer(staking.address, parseUnits("1"));
        await stable
          .connect(investor1)
          .transfer(staking.address, parseUnits("2.5"));
    ...
    describe("convertUsdToToken", () => {
      it("converts a USD value to TAL based on given rate", async () => {
        expect(await staking.convertUsdToToken(parseUnits("1"))).to.equal(
          parseUnits("50")
        );
      });
    });
    ...
    describe("convertTokenToTalent", () => {
      it("converts a TAL value to a talent token based on a given rate", async () => {
        expect(await staking.convertTokenToTalent(parseUnits("5"))).to.equal(
          parseUnits("1")
        );
      });
    });
    ...
    describe("convertTalentToToken", () => {
      it("converts a Talent token value to TAL based on a given rate", async () => {
        expect(await staking.convertTalentToToken(parseUnits("1"))).to.equal(
          parseUnits("5")
        );
      });
    });
    ...
    describe("convertUsdToTalent", () => {
      it("converts a USD value to a talent token based on both given rates", async () => {
        expect(await staking.convertUsdToTalent(parseUnits("2"))).to.equal(
          parseUnits("20")
        );
      });
    });
    ...
    describe("setTokenPrice", () => {
      it("is callable by an admin", async () => {
        const action = await staking
          .connect(owner)
          .setTokenPrice(parseUnits("1"));
      ...
      it("changes the token price", async () => {
        ...
        await staking
          .connect(investor2)
          .stakeStable(talentToken1.address, parseUnits("1"));
        expect(await talentToken1.balanceOf(investor2.address)).to.equal(
          parseUnits("1")
        );
      });
    });
