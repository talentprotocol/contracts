...
describe("VirtualTAL", () => {
  ...
  describe("behaviour", () => {
    ...
    ERC165.supportsInterfaces(builder, [
      "IERC165",
      "IERC20",
      "IAccessControl",
      "IERC1363",
  ...
  describe("functions", () => {
    ...
    describe("admin mint", () => {
      it("works when called by the admin", async () => {
        const action = contract
          .connect(admin)
          .adminMint(investor.address, parseUnits("1"));

        await expect(action).not.to.be.reverted;
        expect(await contract.getBalance(investor.address)).to.equal(
          parseUnits("1")
        );
      });
      ...
      it("adds to previously minted values", async () => {
        ...
        const action = contract
          .connect(admin)
          .adminMint(investor.address, parseUnits("2"));
    ...
    describe("admin burn", () => {
      it("works when called by the admin", async () => {
        ...
        const action = contract
          .connect(admin)
          .adminBurn(investor.address, parseUnits("1"));
      ...
      it("does not work when amount is not enough", async () => {
        ...
        const action = contract
          .connect(admin)
          .adminBurn(investor.address, parseUnits("3"));
      ...
      it("is not callable by a non-admin", async () => {
        const action = contract
          .connect(talent)
          .adminBurn(talent.address, parseUnits("1"));
