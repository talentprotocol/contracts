...
describe("TalentNFT", () => {
  ...
  it("validates if the contract can be deployed", async () => {
    const action = deployContract(creator, Artifacts.TalentNFT, [creator.address, "TALENTNFTs"]);

    await expect(action).not.to.be.reverted;
  });

  const builder = async () => {
    return deployContract(creator, Artifacts.TalentNFT, [creator.address, "TALENTNFTs"]) as Promise<TalentNFT>;
  };
  ...
  describe("testing functions", () => {
    ...
    it("validates invalid account whitelist attempt", async () => {
      expect(talentNFTCollection.whitelistAddress(addressOne.address, 1)).to.be.revertedWith(
        "The tier given needs to be greater than TIERS.PUBLIC_STAGE"
      );
    ...
    it("validates if the minting combination is still available", async () => {
      ...
      await expect(talentNFTCollection.connect(creator).setTokenURI(1, "321", "1-1.png", addressOne.address, 1)).not.to
        .be.reverted;
    ...
    it("validates accountTier for setTokenURI", async () => {
      ...
      await expect(talentNFTCollection.connect(creator).setTokenURI(1, "321", "15-15.png", addressOne.address, 15)).not
        .to.be.reverted;
      expect(await talentNFTCollection.isCombinationAvailable("15-15.png")).to.be.false;
      await expect(talentNFTCollection.connect(creator).setTokenURI(2, "321", "15-15.png", addressTwo.address, 5)).to.be
        .reverted;
    });

    it("validates that user can only mint one token", async () => {
      await expect(talentNFTCollection.connect(addressOne).mint("")).to.be.revertedWith(
        "Minting not allowed with current sender roles"
      );
      await talentNFTCollection.setPublicStageFlag(true);
      await talentNFTCollection.whitelistAddress(addressOne.address, 2);
      await expect(talentNFTCollection.connect(addressOne).mint("")).not.to.be.reverted;
      await expect(talentNFTCollection.connect(addressOne).mint("")).to.be.revertedWith(
        "Address has already minted one Talent NFT"
      );
      expect(await talentNFTCollection.totalSupply()).to.eq(1);
    });
    ...
    it("validates uri change", async () => {
      ...
      await expect(talentNFTCollection.connect(creator).setTokenURI(1, "123", "1-1.png", addressOne.address, 1)).not.to
        .be.reverted;
      expect(await talentNFTCollection.tokenURI(1)).to.eq("123");
      await expect(
        talentNFTCollection.connect(creator).setTokenURI(1, "321", "1-1.png", addressOne.address, 1)
      ).to.be.revertedWith("Metadata was already defined for this token");
    });
    ...
    it("validates uri change invalid calls", async () => {
      ...
      await expect(talentNFTCollection.setTokenURI(9999, "123", "1-1.png", addressOne.address, 1)).to.be.revertedWith(
      ...
      await expect(
        talentNFTCollection.connect(creator).setTokenURI(2, "321", "1-1.png", addressOne.address, 1)
      ).to.be.revertedWith("This combination was already minted");
    });
