...
describe("TalentVaultV3", () => {
  ...
  before(async () => {
    ...
    talentVault = (await deployContract(admin, Artifacts.TalentVaultV3, [
      talentToken.address,
      yieldSource.address,
    ])) as TalentVaultV3;
  ...
  describe("Deployment", async () => {
    ...
    it("Should set the correct initial values", async () => {
      expect(await talentVault.yieldRate()).to.equal(5_00);
    ...
    it("reverts with InvalidAddress when _token given is 0", async () => {
      await expect(
        deployContract(admin, Artifacts.TalentVaultV3, [
  ...
  describe("#name", async () => {
    it("is 'TalentVaultV3' reflects the underlying token name, i.e. of 'TalentProtocolToken'", async () => {
      const name = await talentVault.name();

      expect(name).to.equal("TalentVaultV3");
    });
  });

  describe("#symbol", async () => {
    it("is 'sTALENT3' reflects the underlying token symbol, i.e. of 'TALENT'", async () => {
      const symbol = await talentVault.symbol();

      expect(symbol).to.equal("sTALENT3");
    });
  });
  ...
  describe("Rewards Calculation", async () => {
    ...
    it("Should calculate rewards correctly with standard 5% yield rate", async () => {
      const depositAmount = ethers.utils.parseEther("1000");
      await talentToken.transfer(user1.address, depositAmount);
      await talentToken.connect(user1).approve(talentVault.address, depositAmount);
      await talentVault.connect(user1).deposit(depositAmount, user1.address);
  ...
  describe("#setYieldRate", async () => {
    it("Should allow the owner to update the yield rate", async () => {
      ...
      expect(await talentVault.yieldRate()).to.equal(newYieldRate);
    });
